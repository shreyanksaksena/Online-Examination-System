...
public class ExamService implements IExamService {
    ...
    public void saveExam(ExamDTO dto) throws Exception{
        ...
        if(repo.findExamAlreadyScheduled(user.getId(),category.getId())!=null){
            throw new Exception("Test already scheduled");
        }

        exam.setCreatedby(userService.findByUserId(dto.getCreatedby()).get());
        exam=repo.save(exam);
    ...
    public List<Exam> teacherExams(int userid){
        return repo.findByTeacherUserid(userid);
    }
